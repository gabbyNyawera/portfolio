          ...
          <div ref={containerRef} className="space-y-0">
            {principles.map((principle, index) => (
              <div
                key={principle.title}
                data-index={index}
                className={`border-b border-border/40 py-6 transition-all duration-500 ease-out ${
          ...
          <div ref={containerRef} className="space-y-0">
            {principles.map((principle, index) => (
              <div
                key={principle.title}
                data-index={index}
                className={`border-b border-border/40 py-8 transition-all duration-700 ease-out ${
                  visibleItems.has(index)
                    ? "opacity-100 translate-y-0"
                    : "opacity-0 translate-y-8"
                }`}
              >
                <h3 className="font-serif text-3xl font-light tracking-tight sm:text-4xl">
                  {principle.title}
                </h3>
